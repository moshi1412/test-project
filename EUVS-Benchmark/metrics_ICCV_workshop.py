# This script evaluates rendered images against ground truth images.
...
def evaluate(gt_path, renders_path, dynamic_mask_path, testset_txt_path, output_json_path, visualize_path=None):
    ...
    # Read dynamic masks and images
    dynamic_masks = readMaskWTxt(dynamic_mask_path, testset_txt_path)
    image_paths = readImages(renders_path, gt_path)
    ...
    with torch.no_grad():
        for i, (render_path, gt_path) in enumerate(tqdm(image_paths, desc="Metric evaluation progress")):
            ...
            # Visualization
            if visualize_path:
                save_path = Path(visualize_path) / f"visualization_{i}.png"
        ...
        # Update dictionaries
        full_dict.update({"SSIM": sum(ssims) / len(ssims),
                                            "PSNR": sum(psnrs) / len(psnrs),
                                            "LPIPS": sum(lpipss) / len(lpipss),
                                            "Cos_Similarity": sum(cos_sims) / len(cos_sims)})
        ssim_val = full_dict["SSIM"]
        psnr_val = full_dict["PSNR"]
        lpips_val = full_dict["LPIPS"]
        cos_val = full_dict["Cos_Similarity"]
        final_score = (
            0.25 * ssim_val +
            0.15 * psnr_val +
            0.35 * (1 - lpips_val) +
        ...
        # Final score
        full_dict["Final_Score"] = final_score
...
if __name__ == "__main__":
    ...
    parser.add_argument('--renders_path', '-r', type=str, required=True,
                        help="Path to the rendered outputs for evaluation")
    parser.add_argument('--dynamic_mask_path', '-d', type=str, default=None,
                        help="Path to the dynamic mask (optional)")
    parser.add_argument('--testset_txt_path', '-t', type=str, required=True,
                        help="Path to the .txt file listing the test set files")
    parser.add_argument('--output_json_path', '-o', type=str, required=True,
                        help="Path to save the output evaluation results in JSON format")
    parser.add_argument('--visualize_path', '-v', type=str, default=None,
    ...
    evaluate(
        ...
        dynamic_mask_path=args.dynamic_mask_path, 
        testset_txt_path=args.testset_txt_path, 
