...
# Save the pixel-wise mask for the first class
first_class_mask = masks[0].astype(np.uint8)  # Convert boolean mask to uint8 for saving
first_class_mask_path = os.path.join(OUTPUT_DIR, "first_class_pixelwise_mask.png")
cv2.imwrite(first_class_mask_path, first_class_mask)

masks_output_path = os.path.join(OUTPUT_DIR, "first_class_mask.npy")

# save npy
np.save(masks_output_path, first_class_mask)

print(f"Masks have been saved to {masks_output_path}")
