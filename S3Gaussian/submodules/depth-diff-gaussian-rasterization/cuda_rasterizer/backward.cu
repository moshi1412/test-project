    ...
    float* dL_dcov3D)            // 累加目标（6 个值 per Gaussian）
{
    ...
    if (grad_var == 0.0f) return;

    // 提取视线方向在世界坐标系下的分量（视图矩阵的第三行）
    // 注意 viewmatrix 是列主序？原始代码中 viewmatrix 是 4x4，其中旋转部分为：
    // viewmatrix[0], viewmatrix[1], viewmatrix[2] 是第一行
    // viewmatrix[4], viewmatrix[5], viewmatrix[6] 是第二行
    // viewmatrix[8], viewmatrix[9], viewmatrix[10] 是第三行
    // 根据前向代码中 depth_var 的计算：R 为 viewmatrix[0:3,0:3] 的转置？需要核对。
    // 在前向 preprocessCUDA 中，构造 R 为：
    // glm::mat3 R = glm::mat3(
    //     viewmatrix[0], viewmatrix[1], viewmatrix[2],
    //     viewmatrix[4], viewmatrix[5], viewmatrix[6],
    //     viewmatrix[8], viewmatrix[9], viewmatrix[10]);
    // 即 R 是视图矩阵的左上 3x3 部分（直接取，没有转置），所以 R 将世界点转换到相机坐标。
    // 然后 cov_cam = R * cov_world * R^T。
    // 因此深度方差 = cov_cam[2][2] = (R[2] * cov_world * R[2]^T)。
    // R[2] 是矩阵的第三行，即 (viewmatrix[8], viewmatrix[9], viewmatrix[10])。
    float vx = viewmatrix[8];
    float vy = viewmatrix[9];
    float vz = viewmatrix[10];

    // 计算偏导数
    float d_cov_xx = vx * vx;
    float d_cov_xy = 2.0f * vx * vy;
    float d_cov_xz = 2.0f * vx * vz;
    float d_cov_yy = vy * vy;
    float d_cov_yz = 2.0f * vy * vz;
    float d_cov_zz = vz * vz;

    float grad = grad_var;
    atomicAdd(&dL_dcov3D[6*idx + 0], grad * d_cov_xx);
    atomicAdd(&dL_dcov3D[6*idx + 1], grad * d_cov_xy);
    atomicAdd(&dL_dcov3D[6*idx + 2], grad * d_cov_xz);
    atomicAdd(&dL_dcov3D[6*idx + 3], grad * d_cov_yy);
    atomicAdd(&dL_dcov3D[6*idx + 4], grad * d_cov_yz);
...
__device__ void compute_median_depth_gradient(
    float d,                           // median depth
    float alpha, float mean, float var, // Gaussian parameters
    float dL_ddepth_pixel,            // ∂L/∂d from loss
    float& dL_dalpha,
    float& dL_dmean,
    float& dL_dvar)
{
    float cdf_d = gaussian_cdf(d, mean, var);
    float cdf_0 = gaussian_cdf(0.0f, mean, var);
    float pdf_d = gaussian_pdf(d, mean, var);
    float pdf_0 = gaussian_pdf(0.0f, mean, var);
    float T = expf(-alpha * (cdf_d - cdf_0));
    // ∂T/∂α
    float dT_dalpha = -T * (cdf_d - cdf_0);
    // ∂T/∂mean
    float dT_dmean = -T * alpha * (pdf_d - pdf_0);
    // ∂T/∂var
    float dT_dvar = -T * alpha * ((d - mean) * pdf_d / (2.0f * var) - (0.0f - mean) * pdf_0 / (2.0f * var));
    // ∂T/∂d
    float dT_dd = -T * alpha * pdf_d;
    // ∂d/∂θ = - (∂T/∂θ) / (∂T/∂d)
    float factor = dL_ddepth_pixel / dT_dd;
    dL_dalpha = -factor * dT_dalpha;
    dL_dmean = -factor * dT_dmean;
    dL_dvar  = -factor * dT_dvar;
}
...
renderCUDA(
    ...
    const float* __restrict__ depth_vars,
    const float* __restrict__ final_Ts,
    const uint32_t* __restrict__ n_contrib,
    const float* __restrict__ dL_dpixels,
    const float* __restrict__ dL_dpixel_depths,
    const float* __restrict__ median_left_depth,
    const float* __restrict__ median_right_depth,
    const float* __restrict__ median_left_T,
    const float* __restrict__ median_right_T,
    const uint32_t* __restrict__ median_left_gid,
    const uint32_t* __restrict__ median_right_gid,
	...
	float* dL_ddepthvar)
{
    ...
    __shared__ float collected_depth_vars[BLOCK_SIZE];
    ...
    if (dL_dpixel_depth != 0.0f && local_cnt > 0) {
		...
		float total_cdf_diff = 0.0f;
		for (int i = 0; i < local_cnt; i++) {
			float pdf = gaussian_pdf(d_med, local_depths[i], local_vars[i]);
			sum_alpha_pdf += local_alphas[i] * pdf;
			float cdf_d = gaussian_cdf(d_med, local_depths[i], local_vars[i]);
			float cdf_0 = gaussian_cdf(0.0f, local_depths[i], local_vars[i]);
			total_cdf_diff += local_alphas[i] * (cdf_d - cdf_0);
		}
    ...
    // 然后对左右两个高斯计算梯度（也可以对所有高斯计算，但左右是主要的）
		...
		auto apply_gradient = [&](uint32_t gid, float mean, float var, float alpha) {
			...
			float cdf_0 = gaussian_cdf(0.0f, mean, var);
			float pdf_d = gaussian_pdf(d_med, mean, var);
			float pdf_0 = gaussian_pdf(0.0f, mean, var);
			float dT_dalpha = -T_total * (cdf_d - cdf_0);
			float dT_dmean  = -T_total * alpha * (pdf_d - pdf_0);
			float dT_dvar   = -T_total * alpha * ((d_med - mean) * pdf_d / (2.0f * var) - (0.0f - mean) * pdf_0 / (2.0f * var));
			float dL_dalpha = -factor * dT_dalpha;
			float dL_dmean  = -factor * dT_dmean;
			float dL_dvar   = -factor * dT_dvar;
			atomicAdd(&(dL_dopacity[gid]), dL_dalpha);
			atomicAdd(&(dL_ddepths[gid]), dL_dmean);
			atomicAdd(&(dL_ddepthvar[gid]), dL_dvar);
		};


        // 处理右侧高斯（区间右边界）
        if (right_gid != 0xFFFFFFFF) {
			// 需要知道 right_gid 对应的 mean, var, alpha，可以从全局数组读取（depths, depth_vars, conic_opacity）
			float mean_r = depths[right_gid];
			float var_r = depth_vars[right_gid];
			float alpha_r = conic_opacity[right_gid].w;
			apply_gradient(right_gid, mean_r, var_r, alpha_r);
		}
		if (left_gid != 0xFFFFFFFF && left_gid != right_gid) {
			float mean_l = depths[left_gid];
			float var_l = depth_vars[left_gid];
    ...
    float* dL_ddepthvar)
{
    renderCUDA<NUM_CHANNELS> <<<grid, block>>>(
        ...
        bg_color, means2D, conic_opacity, colors, depths, depth_vars,
        final_Ts, n_contrib,
        dL_dpixels, dL_dpixel_depths,
        median_left_depth, median_right_depth,
        median_left_T, median_right_T,
        median_left_gid, median_right_gid,
	...
	glm::vec4* dL_drot)
{
	...
	if (dL_ddepthvar != nullptr) {
		depthvarToCov3DGradient <<<(P+255)/256, 256>>>(
			P, dL_ddepthvar, viewmatrix, dL_dcov3D);
	}
