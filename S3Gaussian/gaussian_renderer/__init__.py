...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, stage="fine",return_decomposition=False,return_dx=False,render_feat=False,use_fags=False):
    """
    Render the scene. 
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        tanfovx=tanfovx, #水平场视角正切值
        tanfovy=tanfovy,#垂直场视角正切值
        bg=bg_color,#背景颜色 rgb
        scale_modifier=scaling_modifier,#高斯尺寸修正因子
        viewmatrix=viewpoint_camera.world_view_transform.cuda(),#视图变换矩阵（4x4），将世界坐标转换到相机坐标
        projmatrix=viewpoint_camera.full_proj_transform.cuda(),#投影变换矩阵（4x4），将相机坐标转换到屏幕坐标
        sh_degree=pc.active_sh_degree,#Spherical Harmonics 的阶数，控制颜色表达能力
        campos=viewpoint_camera.camera_center.cuda(),#相机在世界坐标系中的位置（3D点）
        prefiltered=False,
        debug=pipe.debug
    )
    ...
    if "coarse" in stage:
        ...
        #使用傅里叶                                                         
        if use_fags:
            dx, dr, ds = pc.fags_deform(xyz, t)
            xyz = xyz + dx
            rot = rot + dr
            scale = scale * (1 + ds)
            lam, beta = pc.fdgk.get_params()
