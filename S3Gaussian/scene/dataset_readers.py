...
def constructCameras_waymo(frames_list, white_background, mapper = {},
    ...
    for idx, frame in enumerate(frames_list):
        ...
        if feat_map_path is not None:
            ...
            # 使用 .copy() 创建可写副本，避免 PyTorch 警告
            features = torch.from_numpy(features.copy()).unsqueeze(0).float()
...
def readWaymoInfo(path, white_background, eval, extension=".png", use_bg_gs=False, 
    ...
    print('num data:', len(img_filepaths))
    if load_feat_map:
    ...
    # storePly(ply_path, xyz, SH2RGB(shs) * 255)
        ...
        for t in trange(0, len(lidar_filepaths), desc="loading lidar", dynamic_ncols=True):
            # 先读取文件并检查实际大小
            lidar_info_raw = np.memmap(
                lidar_filepaths[t],
                dtype=np.float32,
                mode="r",
            )
            total_elements = len(lidar_info_raw)
            file_size_bytes = total_elements * 4  # float32 = 4 bytes
            
            # 尝试推断可能的列数
            possible_cols = []
            for cols in [10, 14, 7, 8, 9, 11, 12, 13, 15]:
                if total_elements % cols == 0:
                    possible_cols.append(cols)
            
            # print(f"\n文件: {lidar_filepaths[t]}")
            # print(f"文件大小: {file_size_bytes} 字节")
            # print(f"总元素数: {total_elements} (float32)")
            # print(f"可能的列数: {possible_cols}")
            # print(f"如果reshape成(-1, 10): {total_elements} / 10 = {total_elements / 10}")
            # print(f"如果reshape成(-1, 14): {total_elements} / 14 = {total_elements / 14}")
            
            # 使用第一个可能的列数，如果没有则使用10
            actual_cols = possible_cols[0] if possible_cols else 10
            # print(f"使用列数: {actual_cols}")
            
            lidar_info = lidar_info_raw.reshape(-1, actual_cols)
